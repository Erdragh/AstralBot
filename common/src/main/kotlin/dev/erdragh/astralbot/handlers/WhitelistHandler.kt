...
object WhitelistHandler {
    ...
    fun writeWhitelistMessage(user: GameProfile): Optional<Component> {
        ...
        if (isDbWhitelisted) return Optional.of(TextComponent(AstralBotTextConfig.WHITELIST_LINKED_NOT_ALLOWED.get().replace("{{name}}", user.name)))
        ...
        for (i in template.indices) {
            ...
            if (discordLink.isNotEmpty() && i + 1 < template.size) {
                ...
                component.append(TextComponent(discordLink).withStyle {it.withClickEvent(ClickEvent(ClickEvent.Action.OPEN_URL, discordLink))})
            }
        }
